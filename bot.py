...
from Bot_Special_Functions import send_photo, send_keyboard, send_url_keyboard, farewell, greeting, \
    check_time
from Bot_DB_Worker import encrypt_password, check_password_correctness, create_user_dictionary, DB, \
    generate_login_password

# from Bot_DB_Classes import encrypt_password, check_password_correctness
...
class History:
    ...
    def start_history(self, message):
        ...
        send_url_keyboard(self.bot, message, "МОСГОРТУР", config.mgt_url,
...
class Services:
    ...
    def services_information(self, message):
        if message.text == "📋Меню":
            ...
            send_url_keyboard(self.bot, message, "Перейти на GLPI", config.glpi_url,
            ...
            sleep(1.5)
            send_url_keyboard(self.bot, message, "Перейти на GLPI", config.glpi_url, config.office_install)
            send_url_keyboard(self.bot, message, "Перейти на Office.com", config.office_url,
            ...
            send_url_keyboard(self.bot, message, "Перейти к ЭДО", config.glpi_url, "Нажмите, чтобы перейти к сервису: ")
            self.more_services(message)
            ...
            send_url_keyboard(self.bot, message, "Перейти на АИС", config.ais_url, "Нажмите, чтобы перейти к сервису: ")
            self.more_services(message)
            ...
            send_url_keyboard(self.bot, message, "Перейти на Helpdesk", config.ais_url,
                              "Нажмите, чтобы перейти к сервису: ")
            self.more_services(message)
...
class Privileges:
    ...
    def start_privileges(self, message):
        if message.text == "📋Меню":
            ...
            msg = send_keyboard(self.bot, message, ["☎Связь", "📚Публикации", "🏀⚽Спорт", "☂Страховка", "🎫Путёвки",
                                                    "🇬🇧Английский", "📋Меню"], "Сотрудникам МОСГОРТУР доступны следующие привилегии",
                                True)
    ...
    def privileges_information(self, message):
        if message.text == "📋Меню":
            ...
            send_url_keyboard(self.bot, message, "Перейти на GLPI", config.glpi_url, config.megafon_access)
            send_url_keyboard(self.bot, message, "МегаФон", config.megafon_url, "Узнать больше на сайте Мегафон:")
            ...
            send_url_keyboard(self.bot, message, "Публикация", config.articles_example_url, "Пример нашей публикации:")
            send_url_keyboard(self.bot, message, "Перейти к библиотеке", config.articles_library_url,
            ...
            sleep(1)
            send_url_keyboard(self.bot, message, "Перейти на сайт", config.english_url1, config.camp_access1)
            ...
            send_url_keyboard(self.bot, message, "SKYENG", config.english_url1, "Перейти на сайт")
            sleep(1.5)
            ...
            send_url_keyboard(self.bot, message, "Запишитесь на первый пробный урок", config.english_url2, config.english_message5)
            self.more_privileges(message)
...
class Sport:
    ...
    def sport_information(self, message):
        if message.text == '🔙Отмена':
            ...
            sleep(1.5)
            send_url_keyboard(self.bot, message, "🗺Карта клубов", config.worldclass_map, config.worldclass_message)
            base = DB()
            ...
            if is_authorized:
                ...
                self.bot.send_message(message.chat.id, config.worldclass_access3)
            sleep(1)
            send_url_keyboard(self.bot, message, "📄Прайс-лист", config.worldclass_pricelist, "💲Корпоративные цены")
            send_url_keyboard(self.bot, message, "Перейти на сайт", config.worldclass_url,
            ...
            sleep(1.5)
            send_url_keyboard(self.bot, message, "🗺Карта клубов", config.xfit_map, config.xfit_message)
            base = DB()
            connection = base.get_connection()
            authorizator = User(connection)
            is_authorized = authorizator.check_authorization(message.chat.id)
            if is_authorized:
                ...
                self.bot.send_message(message.chat.id, config.xfit_access3)
            sleep(1)
            send_url_keyboard(self.bot, message, "Перейти на сайт", config.xfit_url, "Больше о XFit на сайте")
            self.more_sport(message)
    ...
    def get_application(self, message):
        self.user = get_user_information(message)['user_name']
        self.bot.send_message(message.chat.id, '❗Для создания заявки мне потребуется информация❗')
        msg = self.bot.send_message(message.chat.id, 'Введите свой контактный телефон')
    ...
    def get_phone(self, message):
        ...
        if self.club == 'worldclass':
            msg = bot.send_message(message.chat.id, 'Введите Вашу дату рождения:')
        else:
            send_url_keyboard(self.bot, message, "Клубы", 'https://www.xfit.ru/club/',
                              "Выберете клуб, который хотите посещать")
            msg = bot.send_message(message.chat.id, 'Введите название выбранного клуба '
                                                    'и предполагаемую дату посещения (через пробел)')
    ...
    def get_info(self, message):
        if self.club == 'worldclass':
            ...
            self.address = info[0]
            self.date = info[1]
        self.create_application(message)

    def create_application(self, message):
        with open("application_text.txt", "w", encoding="utf-8") as sport_file:
            if self.club == 'worldclass':
                print(config.sport_application_wc.format(self.phone, self.date_of_birth, self.user), file=sport_file)
            else:
...
class Insurance:
    ...
    def start_insurance(self, message):
        ...
        send_url_keyboard(self.bot, message, "PECO", config.insurance_url, config.insurance_message)
    ...
    def insurance_information(self, message):
        if message.text == "🚗ОСАГО":
            ...
            self.bot.send_message(message.chat.id, config.insurance_access)
            ...
            sleep(2.5)
            send_url_keyboard(self.bot, message, "Перейти на сайт", config.dms_url, config.dms_doc)
            self.bot.send_message(message.chat.id, config.insurance_access)
            self.more_insurance(message)
...
class User:
    ...
    def init_table(self):
        ...
        cursor.execute('''CREATE TABLE IF NOT EXISTS users
                            ...
                            user_password VARCHAR(128), user_name VARCHAR(50), user_photo VARCHAR,
                            user_department VARCHAR, user_section VARCHAR,  user_position VARCHAR, self_info VARCHAR,
                            access_level INTEGER, last_seen VARCHAR(20), last_try VARCHAR(20),
    ...
    def check_for_existence_name(self, user_name):
        cursor = self.connection.cursor()
    ...
    def change_password(self, user_login, old_password, new_password):
        ...
        if self.check_for_existence_login(user_login):
    ...
    def get_user_by_name(self, user_name):
        ...
        user_name = ' '.join(name)
        cursor = self.connection.cursor()
        cursor.execute("SELECT * FROM users WHERE user_name = ?", (user_name.strip(),))
        row = cursor.fetchone()
        return row
...
class Admin(User):
    def insert_user(self, user_name, user_login, password, user_photo, user_department, user_section, user_position,
                    access_level):
        cursor = self.connection.cursor()
        password_hash = encrypt_password(password)
        cursor.execute('''INSERT INTO users
                          (user_login, user_password, user_name, user_photo, user_department, user_section,
                          user_position, access_level) VALUES (?,?,?,?,?,?,?,?)''',
                       (user_login, password_hash, user_name, user_photo, user_department, user_section, user_position,
                        access_level))
        cursor.close()
        self.connection.commit()
...
class Administer(Admin):
    ...
    def admin_menu(self, message, bot_message='"Чем я могу помочь?"', admin_help=False):
        if admin_help:
            sleep(1)
            bot.send_message(message.chat.id, config.admin_menu_help_message)
        sleep(1)
    ...
    def admin_menu_functions(self, message):
        if message.text == '📋Меню':
            ...
            keyboard = send_keyboard(self.bot, message, ['🔙Отмена'], 'Введите имя добавляемого пользователя')
            self.bot.register_next_step_handler(keyboard, self.start_insert)
    ...
    def insert_access_level(self, message):
        if message.text == '🔙Отмена':
            ...
            if (self.access_level < self.admin_access < 3) or \
                ...
                self.insert_user(self.user_name, self.login, self.password, self.photo, self.user_department,
                                 self.user_section, self.user_position, self.access_level)
                self.bot.send_message(message.chat.id, 'Пользователь успешно добавлен!')
                user = create_user_dictionary(self.get_user_by_name(self.user_name))
    ...
    def admin_delete_user(self, message):
        if message.text == '🔙Отмена':
            ...
            if str(message.text[0]) in '1234567890':
                ...
                name = message.text
                check = self.check_for_existence_name(name)
                if check:
...
class Mailing:
            ...
            for department in self.departments:
                ...
                if self.departments[department]:
                    for section in self.departments[department]:
                        cursor.execute("SELECT user_id, user_name, user_department, user_section, "
                                       "user_position FROM users WHERE user_department = ? "
                                       "AND user_section = ?", (department, section))
                        row = cursor.fetchall()
                        users_list += '\t\tОтдел:\t{}\n'.format(section)
                        if row:
                            for user in row:
                                users_list += config.user_info.format(user[0], user[1], user[4])
                else:
                    cursor.execute("SELECT user_id, user_name, user_department, user_section, "
                                   "user_position FROM users WHERE user_department = ? ", (department,))
                    users_list += '\nУправление:\t{}\n\n'.format(department)
                    row = cursor.fetchall()
                    if row:
                        section = ''
                        for i in row:
                            if i[3] != section:
                                section = i[3]
                                users_list += '\t\tОтдел:\t{}\n'.format(section)
                            users_list += config.user_info.format(i[0], i[1], i[4])
            self.bot.send_message(message.chat.id, users_list)
    ...
    def create_mailing_list(self, message):
        ...
        if self.users_for_mail:
            for id in self.users_for_mail:
                cursor.execute("SELECT chat_id, user_name FROM users WHERE user_id = ?", (id,))
                row = cursor.fetchone()
                if row[0]:
            ...
            for department in self.departments:
                if self.departments[department]:
                    if self.departments[department]:
                        for section in self.departments[department]:
                            cursor.execute("SELECT chat_id, user_name FROM users WHERE user_department = ? "
                                           "AND user_section = ?", (department, section))
                            raw = cursor.fetchall()
                            if raw:
                                for user in raw:
    ...
    def get_message(self, message):
        if message.text == '🔙Отмена':
            ...
            self.mailing_message = message.text
    ...
    def mailing(self, message):
        ...
        if self.mailing_list:
            ...
            while i < len(self.mailing_list):
                for j in range(29):
                    self.bot.send_message(self.mailing_list[i], self.mailing_message)
    ...
    def show_users_list(self, message, users_list="Вот доступные пользователи:\n",
        ...
        if self.user_department:
            if self.user_section:
                cursor.execute("SELECT user_id, user_name, user_department, user_section, user_position FROM users "
                               "WHERE user_department = ? AND user_section = ? AND access_level <= ?",
                               (self.user_department, self.user_section, self.admin_access))
            else:
                cursor.execute("SELECT user_id, user_name, user_department, user_section, user_position FROM users "
                               "WHERE user_department = ? AND access_level < ?",
            ...
            if self.user_section:
                ...
                if self.admin_access == 3:
                    cursor.execute("SELECT user_id, user_name, user_department, user_section, user_position FROM users "
                                   "WHERE user_id <> ?", (self.admin_info['user_id'],))
                else:
                    cursor.execute("SELECT user_id, user_name, user_department, user_section, user_position FROM users "
                                   "WHERE user_department = ? AND user_section = ? AND access_level < ?",
        ...
        row = cursor.fetchall()
        if row:
            users = sorted(row, key=lambda x: (x[2], x[3], x[1]))
            department = ''
            section = ''
            for user in users:
                if user[2] != department:
                    department = user[2]
                    users_list += '\nУправление:\t{}\n\n'.format(department)
                if user[3] != section:
                    section = user[3]
            ...
            if self.list_type == 'delete_list':
                ...
                msg = send_keyboard(bot, message, [config.user_list, '🔙Отмена'],
                                    'Введите id или ФИО сотрудника (первые буквы БОЛЬШИЕ), о котором хотите узнать')
...
class BotDB:
    ...
    def insert_chat(self, chat_id):
        ...
        new_time = today.strftime("%Y/%m/%d/%H/%M")
...
class Authorization(User):
    ...
    def password_answer(self, message):
        if message.text == '🔙Отмена':
            ...
            if self.check_password_match(self.login, password):
                cursor = self.connection.cursor()
                today = datetime.today()
                new_time = today.strftime("%Y/%m/%d/%H/%M")
                ...
                self.connection.commit()
                cursor.execute("SELECT user_name FROM users WHERE user_login = ?", (self.login,))
                row = cursor.fetchone()
                if row:
                    if len(row[0].split()) == 3:
                        name = ', ' + row[0].split()[1] + ' ' + row[0].split()[2] + '!'
                    else:
                ...
                if self.count > 0:
                    ...
                    if self.return_menu == 'first':
                        start_menu(message, False,
                                   'Исчерпан лимит попыток ввода пароля. Попробуйте ещё раз через минуту.')
                    else:
                        guest_menu(message, False,
                                   'Исчерпан лимит попыток ввода пароля. Попробуйте ещё раз через минуту.')
...
class PersonalAccount(User):
    ...
    def account_menu_functions(self, message):
        if message.text == "📋Меню":
            ...
            if self.information['self_info']:
                bot.send_message(message.chat.id, 'Вот ваша личная информация:')
                old_info_message = self.information['self_info']
            else:
                old_info_message = config.self_information_help_message
            self.bot.send_message(message.chat.id, old_info_message)
            msg = send_keyboard(self.bot, message, ['🔙Отмена'], 'Введите новую')
    ...
    def account_change_login(self, message):
        if message.text == '🔙Отмена':
            ...
            if change:
                ...
                self.bot.send_message(message.chat.id, 'Выс опередили! Такой логин уже существует :(')
    ...
    def account_get_old_password(self, message):
        if message.text == '🔙Отмена':
            ...
            if self.check_password_match(self.information['user_login'], self.old_password):
    ...
    def change_photo(self, message):
        if message.text == '🔙Отмена':
            ...
            try:
                ...
                photo_info = self.bot.get_file(message.photo[len(message.photo) - 1].file_id)
                downloaded_photo = self.bot.download_file(photo_info.file_path)
                if user['user_photo'] != 'photos/Empty.jpg':
...
def keyboard_answer(button):
    ...
    if button.data.split("_")[1] == "Да":
        ...
        connection = base.get_connection()
        authorizator = User(connection)
        is_authorized = authorizator.check_authorization(msg.chat.id)
        if is_authorized:
            base.__del__()
            main_menu(msg)
        else:
...
def guest_menu_functions(message):
    if message.text == "🚪Вход":
        check_authorization(message, 'second')
    elif message.text == "📜История":
        history(message, 'first')
    elif message.text == "💎Привилегии":
        privileges(message, 'first')
    elif message.text == "💬Обратная связь":
        feedback(message, 'second')
...
def menu_functions(message):
    if message.text == "📜История":
        ...
        privileges(message, 'second')
    elif message.text == "💬Обратная связь":
        feedback(message, 'third')
...
def check_authorization_try(message, last_try, menu):
    last_authorization_try = check_time(last_try)[1]
    if last_authorization_try >= 1:
        authorization(message, menu)
    else:
        if menu == 'first':
            start_menu(message, False, '❗Исчерпан лимит попыток ввода пароля. Попробуйте ещё раз через минуту.')
        else:
            guest_menu(message, False, '❗Исчерпан лимит попыток ввода пароля. Попробуйте ещё раз через минуту.')
...
def entrance(message):
    ...
    if is_authorized:
        ...
        authorization_time = check_time(last_seen)[0]
        if authorization_time < 30:
            base.__del__()
            main_menu(message, 'Чем я могу помочь?', False)
        else:
            base.__del__()
            bot.send_message(message.chat.id, 'В целях безопасности, пожалуйста, пройдите автоизацию повторно')
            check_authorization_try(message, last_try, 'first')
    else:
        base.__del__()
        ...
        if not user:
            ...
            user = bot_base.get_chat_info(message.chat.id)
        base.__del__()
        check_authorization_try(message, user[0], 'first')


@bot.message_handler(commands=["menu"])
def check_authorization(message, menu='first'):
    base = DB()
    connection = base.get_connection()
    authorizator = User(connection)
    is_authorized = authorizator.check_authorization(message.chat.id)
    if is_authorized:
        ...
        if authorization_time < 30:
            ...
            check_authorization_try(message, last_try, menu)
    else:
        base.__del__()
        base = DB(config.bot_base_name)
        connection = base.get_connection()
        ...
        if not user:
            ...
            user = bot_base.get_chat_info(message.chat.id)
        base.__del__()
        check_authorization_try(message, user[0], menu)
...
def user_search(message):
    if message.text == '🔙Отмена':
        ...
        if info:
            ...
            name_message = '{}\n\n• Управление: {}\n• Отдел: {}\n• Должность: {}'.format(info['user_name'],
            ...
            base.__del__()
            start_navigation(message, 'Хотите узнать ещё что-нибудь?')
        else:
...
def feedback(message, menu):
    ...
    if menu == 'first':
        msg = send_keyboard(bot, message, ['📋Стартовое Меню'], config.feedback)
    elif menu == 'second':
        msg = send_keyboard(bot, message, ['📋Гостевое Меню'], config.feedback)
    else:
        msg = send_keyboard(bot, message, ['📋Меню'], config.feedback)
...
def sport_application(button):
        if button.data.split("_")[1] == "worldclass":
            sport_obj = Sport(bot, 'first', 'worldclass')
            sport_obj.get_application(button.message)
        else:
            sport_obj = Sport(bot, 'first', 'xfit')
            sport_obj.get_application(button.message)
