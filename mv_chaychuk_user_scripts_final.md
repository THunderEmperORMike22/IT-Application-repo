...
**[ Сценарий 9 – предоставление возможности экспорта ответов пользователя в файл]**
...
**[ Сценарий 16 – Добавление пользователя]**
...
11.	 Бот запрашивает у Администратора поочерёдно управление и отдел добавляемого пользователя (в каждом случае на экран добавляются кнопки с возможностью выбора одного из существующих управлений/отделов).
12.	 Администратор выбирает управление/отдел из существующих или вводит новое(-ый), или отменяет процесс.
13.	 Бот запрашивает у администратора должность добавляемого сотрудника.
14.	 Администратор вводит должность или отменяет процесс.
15.  Бот запрашивает у администратора уровень доступа нового сотрудника и сообщение с объяснением значения этих уровней.
16. Администратор вводит уровень доступа или отменяет процесс.
17.	 Бот добавляет пользователя в базу данных, сообщает об успешном завершении процесса и присылает сообщение с информацией о новом пользователе. После этого запускается сц.15 п.2.
